      ...
      <ScrollView contentContainerStyle={{ padding: spacing.lg, paddingBottom: 100 }}>
        <View style={styles.heroCard}>
          <View style={styles.avatarBig}>
            <Text style={styles.avatarBigText}>{user?.name?.[0]?.toUpperCase() || "?"}</Text>
          </View>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 6, marginTop: spacing.md }}>
            <Text testID="profile-name" style={styles.name}>{user?.name}</Text>
            {user?.verified && <Feather name="check-circle" size={18} color={colors.brand} />}
          </View>
          <View style={styles.locRow}>
            <Feather name="map-pin" size={13} color={colors.muted} />
            <Text testID="profile-locality" style={styles.locText}>{user?.locality} · {user?.city}</Text>
          </View>
        ...
        <View style={styles.statsRow}>
          <View style={styles.statCard}>
            ...
            <Text style={styles.statLabel}>Trust Status</Text>
          </View>
          <View style={styles.statCard}>
        ...
        <View style={styles.menu}>
          ...
          <MenuRow icon="info" label="About Neighbourly" />
        </View>
